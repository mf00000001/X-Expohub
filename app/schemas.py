...
from app.models import UserRole, UserStatus, Gender
...
class RegisterRequest(BaseModel):
    ...
    role: UserRole = Field(
        ...
        description="用户角色：visitor(游客)/exhibitor(展商)/boss(老板)/organizer(主办方)",
    )
    ...
    company: Optional[str] = Field(
        None, max_length=200, description="公司名称（展商/主办方必填）"
    )
    ...
    def validate_company(cls, v: Optional[str], info) -> Optional[str]:
        """展商和主办方必须提供公司名称"""
        if info.data.get("role") in (UserRole.EXHIBITOR, UserRole.ORGANIZER) and not v:
...
class ExhibitorActivityResponse(BaseModel):
    ...
    active_exhibitors: int  # 有展位的展商
    with_products: int  # 有展品的展商
    with_matches: int  # 有采购匹配的展商
    activity_rate: float  # 活跃率
...
class ProcurementStatsResponse(BaseModel):
    ...
    completed: int
    cancelled: int
    match_rate: float  # 匹配率 (matched+completed)/total
    completion_rate: float  # 完成率 completed/total
