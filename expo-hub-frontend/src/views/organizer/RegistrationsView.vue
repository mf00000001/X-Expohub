<template>
  <div class="page-container"><div class="page-header"><h2>报名管理</h2><div>
    ...
    <PaginationBar
      ...
      storage-key="reg-admin-page-size"
      @change="fetch"
    />
  </div>
</template>
<script setup lang="ts">
import { ref, computed, onMounted } from 'vue'; import { useRoute } from 'vue-router'; import { registrationApi } from '@/api/registration'; import type { Registration } from '@/api/registration'; import { exhibitionApi } from '@/api/exhibition'; import LoadingSkeleton from '@/components/LoadingSkeleton.vue'; import EmptyState from '@/components/EmptyState.vue'; import PaginationBar from '@/components/PaginationBar.vue'
const PAGE_SIZE_KEY = 'reg-admin-page-size'
const route = useRoute(); const eid = Number(route.params.id); const regs = ref<Registration[]>([]); const loading = ref(false); const error = ref(''); const exTitle = ref(''); const p = ref(1); const ps = ref(Number(localStorage.getItem(PAGE_SIZE_KEY)) || 20); const total = ref(0); const tp = computed(()=>Math.max(1,Math.ceil(total.value/ps.value)))
function fmt(d:string){ return d?String(d).slice(0,10):'' }
async function fetch(){ loading.value=true; error.value=''; try { const r:any = await registrationApi.getList({ page:p.value, page_size:ps.value, exhibition_id:eid }); regs.value=r.list||[]; total.value=r.total||0 } catch(e:any){ error.value='加载失败（仅展会的组织者或管理员可见）' } finally { loading.value=false } }
