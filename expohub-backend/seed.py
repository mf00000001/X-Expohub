...
try:
    ...
    users = [
        ...
        User(
            ...
            business_license="https://placehold.co/800x400/1e293b/white?text=Canton+Fair",
            nickname="广交会主办方",
            phone="13800010001",
            gender="male",
        ),
    ...
    exhibitions = [
        Exhibition(
            title="第137届中国进出口商品交易会（广交会）",
            description="中国进出口商品交易会（广交会）是中国目前历史最长、规模最大、商品种类最全、到会采购商最多且分布国别地区最广的综合性国际贸易盛会。本届广交会设电子消费品、家用电器、建材五金、日用消费品、纺织服装等展区，汇聚全球优质供应商。",
            cover_image="https://placehold.co/800x400/1e293b/white?text=Canton+Fair",
            start_date="2025-04-15T09:00:00+08:00",
            end_date="2025-05-05T18:00:00+08:00",
            location="广东省广州市海珠区阅江中路382号 中国进出口商品交易会展馆",
            status="published",
        ...
        Exhibition(
            title="第八届中国国际进口博览会（上海进博会）",
            description="中国国际进口博览会（CIIE）是世界上第一个以进口为主题的国家级博览会，是国际贸易发展史上的重大创举。本届进博会设食品及农产品、汽车、技术装备、消费品、医疗器械及医药保健、服务贸易六大展区。",
            cover_image="https://placehold.co/800x400/1e293b/white?text=Canton+Fair",
            start_date="2025-11-05T09:00:00+08:00",
            end_date="2025-11-10T18:00:00+08:00",
            location="上海市青浦区崧泽大道333号 国家会展中心（上海）",
            status="published",
        ...
        Exhibition(
            title="第27届中国国际高新技术成果交易会（深圳高交会）",
            description="中国国际高新技术成果交易会（CHTF）是中国规模最大、最具影响力的科技类展会。本届高交会聚焦新一代信息技术、人工智能、新能源、新材料、生物医药等领域，集中展示全球前沿科技成果。",
            cover_image="https://placehold.co/800x400/1e293b/white?text=Canton+Fair",
            start_date="2025-11-15T09:00:00+08:00",
            end_date="2025-11-19T18:00:00+08:00",
            location="广东省深圳市福田区福华三路 深圳会展中心",
            status="published",
    ...
    products = [
        # --- 华为展品 ---
        Product(booth_id=canton_booths[0].id, exhibition_id=canton_fair.id, exhibitor_id=exh_huawei.id, exhibitor_name=exh_huawei.company, name="华为 Mate 70 Pro", description="搭载麒麟9100芯片，支持卫星通信，XMAGE影像系统", category="智能手机", price=6999.00, unit="台", specs='{"屏幕":"6.82英寸OLED","芯片":"麒麟9100","内存":"12GB+512GB","颜色":["曜金黑","冰霜银"]}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair","https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=500, status="published"),
        Product(booth_id=canton_booths[0].id, exhibition_id=canton_fair.id, exhibitor_id=exh_huawei.id, exhibitor_name=exh_huawei.company, name="华为 MateBook X Pro 2025", description="14.2英寸OLED原色屏，3.1K高分辨率，仅重980g", category="笔记本电脑", price=9999.00, unit="台", specs='{"屏幕":"14.2英寸 OLED 3.1K","CPU":"Intel Ultra 9","内存":"32GB+2TB","重量":"980g"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=200, status="published"),
        Product(booth_id=ciie_booths[1].id, exhibition_id=ciie.id, exhibitor_id=exh_huawei.id, exhibitor_name=exh_huawei.company, name="华为 OceanStor 全闪存存储", description="企业级全闪存存储系统，支持NVMe over Fabrics", category="企业存储", price=250000.00, unit="套", specs='{"容量":"500TB","接口":"NVMe-oF 100GbE","IOPS":"100万+","可用性":"99.9999%"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=20, status="published"),
        Product(booth_id=chtf_booths[0].id, exhibition_id=chtf.id, exhibitor_id=exh_huawei.id, exhibitor_name=exh_huawei.company, name="华为 Pangu 大模型解决方案", description="盘古大模型5.0，支持千亿参数，覆盖NLP/CV/多模态", category="AI/大模型", price=500000.00, unit="套", specs='{"参数规模":"千亿级","支持模态":"文本/图像/语音/视频","部署方式":"公有云/私有化"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=10, status="published"),

        # --- 大疆展品 ---
        Product(booth_id=chtf_booths[1].id, exhibition_id=chtf.id, exhibitor_id=exh_dji.id, exhibitor_name=exh_dji.company, name="大疆 Mavic 4 Pro", description="4/3 CMOS 哈苏相机，全向避障，45分钟续航", category="无人机", price=12888.00, unit="台", specs='{"相机":"4/3 CMOS Hasselblad","续航":"45分钟","图传":"O4 20km","避障":"全向双目"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=300, status="published"),
        Product(booth_id=chtf_booths[1].id, exhibition_id=chtf.id, exhibitor_id=exh_dji.id, exhibitor_name=exh_dji.company, name="大疆 RoboMaster EP Core", description="教育机器人，支持Python编程与AI扩展", category="教育机器人", price=5999.00, unit="台", specs='{"控制方式":"Python/Scratch","传感器":"红外/陀螺仪/视觉","扩展":"AI模块兼容"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=150, status="published"),

        # --- 海尔展品 ---
        Product(booth_id=canton_booths[1].id, exhibition_id=canton_fair.id, exhibitor_id=exh_haier.id, exhibitor_name=exh_haier.company, name="海尔智慧家庭套装", description="全屋智能家电套装：冰箱+洗衣机+空调+热水器，支持U+互联", category="智能家居", price=25999.00, unit="套", specs='{"包含":"冰箱/洗衣机/空调/热水器","互联":"U+智慧平台","能效":"一级能效"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=100, status="published"),
        Product(booth_id=canton_booths[1].id, exhibition_id=canton_fair.id, exhibitor_id=exh_haier.id, exhibitor_name=exh_haier.company, name="海尔鲜风机", description="3D环绕送风，56°C高温自清洁，一级变频节能", category="空调", price=3699.00, unit="台", specs='{"匹数":"1.5匹","能效":"一级变频","自清洁":"56°C高温","送风":"3D环绕"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=500, status="published"),

        # --- 小米展品 ---
        Product(booth_id=canton_booths[4].id, exhibition_id=canton_fair.id, exhibitor_id=exh_xiaomi.id, exhibitor_name=exh_xiaomi.company, name="小米 15 Ultra", description="徕卡光学Summilux镜头，骁龙8 Gen 4，小米HyperOS", category="智能手机", price=6499.00, unit="台", specs='{"屏幕":"6.73英寸 LTPO 2K","芯片":"骁龙8 Gen 4","相机":"徕卡1英寸主摄","电池":"5500mAh"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=800, status="published"),
        Product(booth_id=ciie_booths[4].id, exhibition_id=ciie.id, exhibitor_id=exh_xiaomi.id, exhibitor_name=exh_xiaomi.company, name="小米 SU7 Ultra", description="纯电轿跑，零百加速2.78秒，800V高压平台", category="智能电动汽车", price=299900.00, unit="辆", specs='{"零百加速":"2.78秒","续航":"830km CLTC","平台":"800V高压","智驾":"Xiaomi Pilot Max"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=50, status="published"),

        # --- 西门子展品 ---
        Product(booth_id=ciie_booths[0].id, exhibition_id=ciie.id, exhibitor_id=exh_siemens.id, exhibitor_name=exh_siemens.company, name="西门子 SIMATIC S7-1500", description="高级PLC控制器，支持工业物联网和边缘计算", category="工业自动化", price=15000.00, unit="台", specs='{"CPU":"1518-4 PN/DP","通信":"PROFINET/OPC UA","编程":"TIA Portal V19","冗余":"支持"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=80, status="published"),
        Product(booth_id=ciie_booths[0].id, exhibition_id=ciie.id, exhibitor_id=exh_siemens.id, exhibitor_name=exh_siemens.company, name="西门子 MindSphere", description="工业物联网即服务(IoTaaS)平台，数据驱动的数字化解决方案", category="工业物联网", price=200000.00, unit="年/许可", specs='{"部署":"SaaS/私有化","协议":"MQTT/OPC UA/HTTP","分析":"AI/ML内置","安全":"端到端加密"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=30, status="published"),
        Product(booth_id=chtf_booths[9].id, exhibition_id=chtf.id, exhibitor_id=exh_siemens.id, exhibitor_name=exh_siemens.company, name="西门子 Xcelerator 数字孪生平台", description="端到端数字孪生解决方案，覆盖产品设计到运维全生命周期", category="数字孪生", price=350000.00, unit="年/许可", specs='{"覆盖":"设计/仿真/制造/运维","集成":"PLM/MES/IoT","协同":"云端多用户","格式":"支持30+CAD"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=15, status="published"),

        # V2.3: 多行业展品补充
        Product(booth_id=canton_booths[2].id, exhibition_id=canton_fair.id, exhibitor_id=exh_haier.id, exhibitor_name=exh_haier.company, name="LED智慧路灯", description="智能调光+光伏供电+5G微基站一体化路灯", category="照明", price=8800.00, unit="套", specs='{"功率":"200W LED","供电":"光伏+市电双模","控制":"NB-IoT远程","高度":"8米"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=200, status="published"),
        Product(booth_id=canton_booths[3].id, exhibition_id=canton_fair.id, exhibitor_id=exh_xiaomi.id, exhibitor_name=exh_xiaomi.company, name="电动自行车电池包", description="48V20Ah锂电包，适用新国标电动自行车", category="车辆及配件", price=1299.00, unit="组", specs='{"电压":"48V","容量":"20Ah","电芯":"21700三元锂","循环":"800次+"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=1000, status="published"),
        Product(booth_id=ciie_booths[2].id, exhibition_id=ciie.id, exhibitor_id=exh_siemens.id, exhibitor_name=exh_siemens.company, name="工业级电动扳手", description="无刷电机+扭矩精准控制，适用于汽车装配线", category="五金工具", price=3200.00, unit="把", specs='{"扭矩":"500-3000Nm","精度":"±3%","电池":"20V/5Ah锂电","重量":"3.8kg"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=150, status="published"),
        Product(booth_id=ciie_booths[3].id, exhibition_id=ciie.id, exhibitor_id=exh_dji.id, exhibitor_name=exh_dji.company, name="CNC五轴加工中心", description="高精度五轴联动，适用航空航天精密零件加工", category="机械", price=850000.00, unit="台", specs='{"轴数":"5轴联动","精度":"±0.005mm","转速":"24000rpm","工作台":"Φ800mm"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=5, status="published"),
        Product(booth_id=ciie_booths[5].id, exhibition_id=ciie.id, exhibitor_id=exh_haier.id, exhibitor_name=exh_haier.company, name="环保隔热墙体板", description="聚氨酯+岩棉复合板，A级防火，适用于装配式建筑", category="建材", price=168.00, unit="㎡", specs='{"厚度":"75mm","防火":"A级","导热系数":"≤0.022","规格":"1200×3000mm"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=5000, status="published"),
        Product(booth_id=chtf_booths[2].id, exhibition_id=chtf.id, exhibitor_id=exh_huawei.id, exhibitor_name=exh_huawei.company, name="碳中和碳管理SaaS平台", description="企业碳排放核算+碳足迹追踪+碳交易辅助决策", category="能源", price=98000.00, unit="年/许可", specs='{"标准":"ISO 14064/GHG Protocol","覆盖":"Scope 1/2/3","功能":"核算/报告/交易","部署":"公有云SaaS"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=50, status="published"),
        Product(booth_id=canton_booths[6].id, exhibition_id=canton_fair.id, exhibitor_id=exh_xiaomi.id, exhibitor_name=exh_xiaomi.company, name="智能保温杯", description="OLED温度显示+APP饮水提醒+12小时保温", category="日用消费品", price=199.00, unit="个", specs='{"容量":"450ml","材质":"316不锈钢","保温":"12小时","连接":"蓝牙5.3"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=3000, status="published"),
        Product(booth_id=canton_booths[7].id, exhibition_id=canton_fair.id, exhibitor_id=exh_haier.id, exhibitor_name=exh_haier.company, name="定制企业礼品套装", description="笔记本+保温杯+充电宝+U盘 商务四件套定制", category="礼品", price=128.00, unit="套", specs='{"包含":"笔记本/保温杯/充电宝/U盘","定制":"激光Logo","起订":"100套","交货":"7天"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=500, status="published"),
        ...
        Product(booth_id=ciie_booths[7].id, exhibition_id=ciie.id, exhibitor_id=exh_xiaomi.id, exhibitor_name=exh_xiaomi.company, name="碳纤维安全鞋", description="轻量化碳纤维包头+防刺穿中底+防滑大底", category="鞋类", price=369.00, unit="双", specs='{"标准":"EN ISO 20345 S3","包头":"碳纤维","中底":"凯夫拉防刺","重量":"480g/只"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=2000, status="published"),
        Product(booth_id=canton_booths[8].id, exhibition_id=canton_fair.id, exhibitor_id=exh_haier.id, exhibitor_name=exh_haier.company, name="3D浮雕装饰画", description="UV打印+手工上色，现代简约/新中式风格", category="家居装饰品", price=299.00, unit="幅", specs='{"工艺":"UV打印+手工上色","尺寸":"60×80cm","风格":"现代/新中式","材质":"油画布+松木框"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=400, status="published"),
        Product(booth_id=ciie_booths[8].id, exhibition_id=ciie.id, exhibitor_id=exh_siemens.id, exhibitor_name=exh_siemens.company, name="冻干即食燕窝", description="马来西亚溯源燕窝，FD航天冻干技术，银碗即食", category="食品", price=598.00, unit="盒/6碗", specs='{"原料":"马来西亚溯源燕窝","工艺":"FD冻干","规格":"6碗/盒","保质期":"18个月"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=600, status="published"),
        Product(booth_id=chtf_booths[3].id, exhibition_id=chtf.id, exhibitor_id=exh_huawei.id, exhibitor_name=exh_huawei.company, name="AI辅助诊断系统", description="基于深度学习的CT/MRI影像辅助诊断，已获NMPA三类证", category="医药及医疗保健", price=500000.00, unit="套", specs='{"认证":"NMPA三类/CE MDR","模态":"CT/MRI/X-Ray","病种":"肺结节/骨折/脑出血","准确率":">95%"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=8, status="published"),
        Product(booth_id=chtf_booths[4].id, exhibition_id=chtf.id, exhibitor_id=exh_dji.id, exhibitor_name=exh_dji.company, name="展位搭建一站式服务", description="设计+搭建+拆除全包，100+套方案可选", category="综合服务", price=15000.00, unit="次", specs='{"服务":"设计/搭建/拆除","方案":"100+模板","面积":"9-36㎡","周期":"布展前3天"}', images='["https://placehold.co/800x400/1e293b/white?text=Canton+Fair"]', stock=50, status="published"),
    ]
