...
角色体系（五类）：
- visitor:  游客 — 浏览展会、收藏
- buyer:    买家 — 注册展会、发布采购需求
- exhibitor: 展商 — 报名展会、管理展位/展品
- organizer: 主办方 — 管理展会、审核报名、直接发布
- boss:     老板 — @deprecated 已弃用，保留仅为向后兼容。主办方直接发布展会，不再需要老板审批环节
"""
...
ROLE_PERMISSIONS: dict[UserRole, set[Permission]] = {
    ...
    # @deprecated BOSS 角色已弃用，保留仅为向后兼容。
    # 主办方现在直接发布展会，不再需要老板审批环节。
    UserRole.BOSS: {
        # 老板/高层：全局查看、数据分析、审批
        Permission.USER_READ,
        Permission.USER_UPDATE,
...
# @deprecated 仅老板可访问 — 已弃用，保留仅为向后兼容。
# 主办方现在直接发布展会，不再需要老板审批环节。
require_boss = require_role(UserRole.BOSS)

# 游客或买家可访问
require_visitor_or_buyer = require_role(UserRole.VISITOR, UserRole.BUYER)

# 主办方或老板可访问（管理后台）— 保留 BOSS 仅为向后兼容
...
require_any_user = require_role(
    UserRole.VISITOR, UserRole.BUYER, UserRole.EXHIBITOR, UserRole.ORGANIZER, UserRole.BOSS
)
